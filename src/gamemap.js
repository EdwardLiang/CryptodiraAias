class GameMap {
    ...
    checkMovable(x, y, level, c){
        ...
        if(!this.checkWater(x, y, level) && c instanceof WaterCreature){
            return false;
        }
    ...
    canGoUp(x, y, z){
        if (this.levels[z].blocks[x][y] instanceof StaircaseUpBlock){
            return true;
        }
        return false;
    }
    ...
    canGoDown(x, y, z){
        if (this.levels[z].blocks[x][y] instanceof StaircaseDownBlock){
            return true;
        }
        return false;
    }
    ...
    creaturesAct(){
        for(let j = 0; j < this.levels.length; j++){
            for(let i = 0; i < this.levels[j].creatures.length; i++){
                ...
                let cDir = CDIRS[dir];
                Game.engine.addEvent(this.levels[j].creatures[i].move(cDir));
            }
        }
    }
